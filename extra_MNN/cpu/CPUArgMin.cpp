...
#include "backend/cpu/CPUArgMin.hpp"
#include <float.h>
...
CPUArgMin::CPUArgMin(Backend *backend, ArgMinOrMax mode, int topk, int outMaxVal, int softmaxThreshold, int axis)
    : Execution(backend), mTopk(topk), mOutMaxVal(outMaxVal), mSoftmaxThreshold(softmaxThreshold), mAxis(axis), mMode(mode) {
    // nothing to do
}

ErrorCode CPUArgMin::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    if (mFromNHWC) {
        ...
        // see file: tools/converter/source/caffe/ArgMin.cpp
        } else if (mAxis != 10000) {
            const int dimensions = input->dimensions();
            for (int i = 0; i < mAxis; ++i) {
                mNum = mNum * input->length(i);
            ...
            MNN_PRINT("error in ArgMin, not implemented error.");
            MNN_ASSERT(false);
        }
    }
...
ErrorCode CPUArgMin::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    auto input  = inputs[0];
    auto output = outputs[0];
...
#define element_value(ele) (std::get<1>(ele))
    ...
    if (mFromNHWC) {
        ...
        MNN_ASSERT(mMode == ArgMin); // caffe does not have argmin layer
...
class CPUArgMinCreator : public CPUBackend::Creator {
public:
    virtual Execution *onCreate(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs,
                                const MNN::Op *op, Backend *backend) const {
        auto ArgMin = op->main_as_ArgMax();
        if (op->type() == OpType_ArgMin) {
            return new CPUArgMin(backend, CPUArgMin::ArgMinOrMax::ARGMIN,
                    ArgMin->topK(), ArgMin->outMaxVal(), ArgMin->softmaxThreshold(), ArgMin->axis());
        } 
    }
};
REGISTER_CPU_OP_CREATOR(CPUArgMinCreator, OpType_ArgMin);
} // namespace MNN
