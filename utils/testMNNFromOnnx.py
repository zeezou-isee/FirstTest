...
class TestModel():
    ...
    def __run_mnn(self,mnn_model_path:Path):
        ...
        model_file = self.modelName
        os.makedirs(mnn_model_path.parent, exist_ok=True)

        convert = f'LD_LIBRARY_PATH="$PWD/express:$PWD/tools/converter:$PWD:$LD_LIBRARY_PATH" {mnnconvert_name} -f ONNX --bizCode MNN --modelFile {model_file} --MNNModel {mnn_model_path} --keepInputFormat=1 --testdir onnx'
    ...
    def __run_onnx(self):
        ...
        for inputVar in ort_session.get_inputs():
            ...
            if inputVar.type.find("int64") >= 0:
                ...
                # Float
                inputs[inputVar.name] = np.random.uniform(0.1, 1.2, shapes).astype(np.float32)  # 可修改随机输入的分布范围
    ...
    def Test(self,mnn_model_path):
        self.__run_onnx()
        res = self.__run_mnn(mnn_model_path)
        return res
